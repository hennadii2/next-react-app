...
const AdSpaceSquare = ({ caption, description, premiumAdverts }) => {
	...
	return (
		<section className='small-section slick-15 pt-0 produce'>
			<h4 className='mb-4-5 mt-4 section-title text-center'>
				{caption} {description}
			</h4>
			{premiumAdverts.length < 4 ? (
				<Row>
					{premiumAdverts.map((advert) => {
						if (
							!['Active', 'Reactivated'].includes(advert.statusISLIST_Draft_Active_Deactivated_Reactivated_Archived)
						) {
							return null
						}
						return (
